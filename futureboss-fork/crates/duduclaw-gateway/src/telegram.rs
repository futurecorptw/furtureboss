    ...
    loop {
        ...
        if let Some(updates) = data.result {
            for update in updates {
                ...
                // Show a "typing…" indicator while the agent generates the reply.
                // Telegram clears it after ~5s, so refresh every 4s until we abort.
                let typing_task = {
                    let tclient = client.clone();
                    let tapi = api_base.clone();
                    let tchat = chat_id;
                    let tthread = thread_id;
                    tokio::spawn(async move {
                        loop {
                            send_typing(&tclient, &tapi, tchat, tthread).await;
                ...
                // Reply ready — stop the typing indicator.
                typing_task.abort();
...
/// Send a single Telegram "typing…" chat action. Telegram auto-clears it after
/// ~5 seconds, so callers that wait longer should re-send periodically.
...
/// Best-effort Markdown → plain text. We send with `parse_mode` disabled, so any
/// leftover markup would otherwise show up literally (`**bold**`, `### Title`…).
/// Deliberately conservative: only strips the markup LLMs emit most and never
/// touches lone `*` / `_` (bullet lists, file names) to avoid mangling real text.
fn strip_markdown(input: &str) -> String {
    ...
    for raw in input.lines() {
        ...
        // Drop ``` code-fence lines, keep the code body verbatim.
        ...
        // Accumulate Markdown table rows; render the whole block when it ends.
        // Telegram supports no table mode at all, so we convert to plain lines.
        if trimmed.starts_with('|') {
        ...
        // Drop horizontal rules ("---", "***", "___") — they render as noise.
        if is_hr(trimmed) {
            continue;
        }
...
/// A line that's only dashes / asterisks / underscores (≥3) — a Markdown rule.
fn is_hr(s: &str) -> bool {
    let body: String = s.chars().filter(|c| !c.is_whitespace()).collect();
    body.len() >= 3
        && (body.chars().all(|c| c == '-')
            || body.chars().all(|c| c == '*')
            || body.chars().all(|c| c == '_'))
}
...
/// The `|---|:--:|` divider row between a table header and its body.
...
/// Render accumulated Markdown table rows as plain text Telegram can show.
/// 2-column tables become "key：value" bullets (header dropped); wider tables
/// keep every row, cells joined by " ｜ ". A pipe block with no divider row is
/// left untouched (probably not a real table).
fn flush_table(table: &mut Vec<String>, out: &mut Vec<String>) {
    ...
    if !rows.iter().any(|r| is_separator_row(r)) {
        out.extend(rows);
        return;
    }
    ...
    if cols == 2 {
        for (idx, row) in parsed.iter().enumerate() {
            if idx == 0 {
                continue; // header labels are usually self-evident
            }
            let k = row.first().cloned().unwrap_or_default();
            let v = row.get(1).cloned().unwrap_or_default();
            if k.is_empty() && v.is_empty() {
                continue;
            }
            rendered.push(format!("• {k}：{v}"));
        }
        if rendered.is_empty() {
            if let Some(h) = parsed.first() {
                rendered.push(format!("• {}", h.join("：")));
            }
        }
    } else {
        for (idx, row) in parsed.iter().enumerate() {
...
async fn send_reply(
    ...
    // Drop Markdown markup (we send with parse_mode disabled) then split.
